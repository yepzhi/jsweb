...
> **ATENCIÓN AGENTES IA:** Este documento describe el flujo de trabajo para despliegues automatizados en Vercel. **Léelo y aplícalo en todos los commits.**
...
- La aplicación (Next.js) está **EN LA RAÍZ** del repositorio `/Users/yepz/JSweb/`.
- El **Root Directory en Vercel** se configuró como `.` (raíz del proyecto).
...
Cuando realices cambios como Agente IA en este repositorio, **DEBES** seguir este flujo al pie de la letra para evitar que el CI/CD en Vercel falle:

1. **Verificar que compila localmente antes de hacer commit:**
   Ejecuta `npm run build` desde la raíz del proyecto. NUNCA hagas commit si el build falla. Vercel rechazará el despliegue automático de todas formas.
...
3. **Flujo de Command Line (si el build fue exitoso):**
   ```bash
   cd /Users/yepz/JSweb  # Estar en la raíz del proyecto
   git add .
...
- Vercel marcaba errores (Exit Code 1) porque el agente hacía deploy manual con `vercel --prod` en vez de simplemente hacer `git push` y dejar que el CI/CD de Vercel maneje el build limpio.
...
2. Haz `npm run build` desde la raíz para validar.
3. Haz `git commit & push`.
4. Observa el estatus en Vercel, no intentes hacer force-deploy con Vercel CLI a menos que esté estrictamente solicitado.
